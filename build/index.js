...
var ShellGame = React.createClass({displayName: 'ShellGame',
  ...
  render: function () {
    ...
    return (
      React.DOM.div(null, 
        overlay, 
        React.DOM.div({className: boardClasses}, 
         cups, " ", ball, 
         React.DOM.span({className: "level"}, " Level ", current_level+1)
        )
      )
    );
  }
...
var CupView = React.createClass({displayName: 'CupView',
  render: function () {
    ...
    return (
      React.DOM.span({className: classes, onClick: clickHandler, onTouchEnd: clickHandler, key: cup.id})
    );
  }
});
...
React.renderComponent(ShellGame(null), document.getElementById('content'));